...
# This will watch the /certificates folder and run nginx -s reload whenever there are some changes.
...
while true
do
        inotifywait -e create -e modify -e delete -e move -r --exclude "\\.certbot\\.lock|\\.well-known" "/certificates/$LETSENCRYPT_MODE"
        echo "Changes noticed in /certificates"

        echo "Waiting 5s for additional changes"
        sleep 5
