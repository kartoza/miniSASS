...
if [ ! -f "/certificates/autoissued/privkey.pem" ] || [[ $(find /certificates/autoissued/privkey.pem -mtime +365 -print) ]]; then
        echo "Auto issued certificate does not exist or is too old, we generate one"
        mkdir -p "/certificates/autoissued/"
        openssl req -x509 -nodes -days 1825 -newkey rsa:2048 -keyout "/certificates/autoissued/privkey.pem" -out "/certificates/autoissued/fullchain.pem" -subj "/CN=${HTTP_HOST:-null}"
else
        echo "Auto issued certificate already exists"
fi
...
if [ -f "/certificates/$LETSENCRYPT_MODE/live/$HTTPS_HOST/fullchain.pem" ] && [ -f "/certificates/$LETSENCRYPT_MODE/live/$HTTPS_HOST/privkey.pem" ]; then
        echo "Certbot certificate exists, we symlink to the live cert"
        ln -sf "/certificates/$LETSENCRYPT_MODE/live/$HTTPS_HOST" /certificate_symlink
else
...
envsubst '\$HTTP_PORT \$HTTPS_PORT \$HTTP_HOST \$HTTPS_HOST \$RESOLVER' < /etc/nginx/minisass.conf.envsubst > /etc/nginx/conf.d/minisass.conf
