...
mkdir -p "${CERT_DIR}/$LETSENCRYPT_MODE"
chmod o+rw "${CERT_DIR}"
...
if [ "$LETSENCRYPT_MODE" == "staging" ]; then
    ...
    certbot --config-dir "${CERT_DIR}/$LETSENCRYPT_MODE" certonly --webroot -w "${CERT_DIR}" -d "$HTTPS_HOST" -m "$ADMIN_EMAIL" --agree-tos --non-interactive --staging --server https://acme-staging-v02.api.letsencrypt.org/directory
elif [ "$LETSENCRYPT_MODE" == "production" ]; then
    printf "\nTrying to get PRODUCTION certificate\n"
    certbot --config-dir "${CERT_DIR}/$LETSENCRYPT_MODE" certonly --webroot -w "${CERT_DIR}" -d "$HTTPS_HOST" -m "$ADMIN_EMAIL" --agree-tos --non-interactive --server https://acme-v02.api.letsencrypt.org/directory
else
